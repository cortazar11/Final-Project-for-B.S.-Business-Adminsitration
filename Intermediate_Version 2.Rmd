...
\
Como vemos las únicas estimaciones VaR en que los valores del test de Kupiec son menores que los valores críticos (o sea,los percentiles calculados en la sección de resultados empíricos) y que por tanto pasan los test de hipótesis, son las correspondiente a la distribución normal. 

En términos de las funciones de pérdida vemos que para un coeficiente de riesgo del 5%, la distribución de error generalizada provee las menores pérdidas y la t-student las mayores. Para un coeficiente de riesgo del 1% la función de error generalizada asimétrica provee las pérdidas menores y la normal las mayores.

Asi pues las distribuciones de error generalizado aportarían mejor modelo si pasaran los tests. La única distribución que pasa los tests, la normal, no aporta el mejor modelo en términos de función de pérdida.

A continuacion pasamos a analizar cada uno de los tests.
\
...
```{r ,chunk1, fig.width=13, echo=FALSE, cache=TRUE}
...
```{r chunk2, fig.width=13,echo=FALSE,  cache=TRUE}
...
perdidaStudent99<-round(mean(data$LFStudent99),digits = 5)
...
```{r chunk3, fig.width=13,echo=FALSE,  cache=TRUE}
...
```{r chunk4,fig.width=13,echo=FALSE,  cache=TRUE}
...
De los resultados  presentados en este trabajo podemos concluir que la distribución normal supera la distribución t-student, la de error generalizado y la de error generalizado asimétrico, en ajustarse a los retornos financieros y en predecir el VaR del Ibex35 en el periodo comprendido entre 2000 y 2019, ya que es la única que pasa el test de hipótesis. Sin embargo esta superioridad no es muy clara, ya que en cuanto a los modelos de la función de pérdida ganan las de error generalizado y error generalizado asimétrica para coeficientes de riesgo del 5% y 1% respectivamente. La distribución perdedora claramente es la t-student, que no pasa los tests de hipótesis y sus fuciones de pérdida dan valores elevados.

Asi pues hallamos evidencia en favor de la distribución normal particularmente en relación con la t-student. Sin embargo podemos mantener la atención en las distribuciones de error generalizado y error generalizado asimétrico, ya que sus modelos de función de peŕdida son mejores.
...
```{r}
save.image(file="/home/miguel/UNED_2021/Proyecto/R Language/.RData")
```
